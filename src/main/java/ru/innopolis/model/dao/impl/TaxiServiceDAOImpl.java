...
import ru.innopolis.model.dao.api.TaxiServiceDAO;
...
/**
 * Implements the TaxiService DAO interface.
 * Communicates with the relation TaxiService in the database.
 *
 * @author      Ilya Borovik
 * @version     1.0
 * @see         DAO
...
public class TaxiServiceDAOImpl implements TaxiServiceDAO {
    static {
        PropertyConfigurator.configure(TaxiServiceDAOImpl.class.getClassLoader().getResource("log4j.properties"));
    }

    private static final Logger LOGGER = LogManager.getLogger(TaxiServiceDAOImpl.class);
    ...
    public List<TaxiService> getAll() throws DAOException {
        LOGGER.debug("Getting all taxi ru.innopolis.services ");
        ...
        try (Connection connection = DataSourceFactory.getDataSource().getConnection();
            ...
            LOGGER.error(e.getMessage(), e);
            throw new DAOException("Cannot get all taxi ru.innopolis.services from the database.");
        }

        LOGGER.debug("Got all taxi ru.innopolis.services");
    ...
    public int update(TaxiService taxiService) throws DAOException {
        ...
        LOGGER.info(String.format("Updated fields of %d taxi ru.innopolis.services", count));
